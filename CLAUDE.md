...
- **字段命名**：JSON 输出 snake_case (`is_free`, `free_mechanism`)，TS 内部 camelCase (`isFree`, `freeMechanism`)。映射在 `types.ts:toOpenAICompatible`。
...
- `priceInput === undefined && priceOutput === undefined` 表示未知，**不要当 0 处理**（影响 `inferTier` / `inferSpeed` 的价格启发式）
- 加新 provider 后，记得 `evaluator.ts:PROVIDER_META` 补 `displayName` / `website` / `apiBaseUrl`
- `capability-cache.json` 每次跑都被整条覆盖（write-through），不再 write-once；不要依赖它保存"首次发现"的快照
