...
`is_free = true`：在某种条件下（速率限制 / 配额 / 试用 credits）可以**不付费**调用 API。无论是否永久免费，只要存在不付费可用窗口都算。

具体机制由 `free_mechanism` 字段描述（`permanent` / `rate-limited` / `daily-tokens` / `monthly-tokens` / `trial-credits` / `preview`）。

> Gitee「体验」（原 `is_experienceable=true`）= 付费但开放调用，**不是免费** → `is_free=false`。
...
- 已删除字段（不要恢复）：`billing_mode`、`free_tier`、`is_experienceable`、`free_kind`、`rate_limits`
